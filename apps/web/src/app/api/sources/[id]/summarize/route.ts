...
const SUMMARY_TIMEOUT_MS = 370_000;
...
export async function POST(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    response = await fetch(`${apiBase}/api/sources/${encodeURIComponent(id)}/summarize`, {
      method: 'POST',
      cache: 'no-store',
    ...
    if (error instanceof DOMException && error.name === 'TimeoutError') {
      return proxyError(504, 'AI_TIMEOUT', 'AI 요약 요청 시간이 초과되었습니다.');
    }
    return proxyError(502, 'API_PROXY_FAILED', 'AI 요약 요청 연결이 중단되었습니다.');
  }
