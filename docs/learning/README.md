...
이 폴더는 SourceWiki 프로젝트를 이해하기 위한 학습용 문서입니다. 처음 보는 사람이 프로젝트 기반부터 회원가입, 이메일 인증, JWT 로그인, 자료·댓글 CRUD까지 단계적으로 따라갈 수 있도록 쉬운 설명을 기준으로 정리했습니다.
...
이 프로젝트는 `pnpm`으로 여러 Node 패키지를 한 저장소에서 관리하고, Docker로 인프라를 실행하며, Web과 API가 공유 schema를 기준으로 이메일 인증, 회전형 JWT session, 자료·댓글 CRUD를 처리합니다.
